...
class EasyRisUnitTest(unittest.TestCase):
    ...
    def setUp(self, n_loaded=5, **kwargs):
        ...
        from qScience.database.base import parse_db_config, \
                                            restore_db,\
                                            easyris_connect
...
class EasyRisAppTest(unittest.TestCase):
    ...
    def setUp(self, n_loaded=5, **kwargs):
        from qScience.database.base import restore_db
        db_filename = "config/database_test.cfg"
        easyr = create_app(database_cfg=db_filename, logger=False)
...
class EasyRisTest(unittest.TestCase):
    ...
    def test_home(self):
        rv = self.app.get('/')
        assert 'Flask is on fire!' == rv.data
